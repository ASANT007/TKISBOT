...
<h2>Manage ADService details</h2>
 ...
 <table border="0" align="center" cellpadding="0" cellspacing="0" class="table tbl-report table-bordered table-striped user-dashboard">
      ...
      <thead style="position: sticky;top: 0" class="thead-dark">
        <tr>
          ...
          <th align="center" valign="middle" class="table-heading header">User Id</th>
          <th align="center" valign="middle" class="table-heading header">LDAP URL</th>
          <th align="center" valign="middle" class="table-heading header">Status</th>
          
        </tr>
      </thead>
      <c:if test="${not empty adServiceList}">
      <tr>
      <c:forEach var="obj" items="${adServiceList}" varStatus="theCount">
       <c:set var="adServiceId" value="${obj.serviceId}"></c:set>
         <% String encADServiceId=encdec.Encrypt(Integer.toString((int)pageContext.getAttribute("adServiceId"))); %>
      <tr valign="top">
      	<td class="text-center"><a href=javascript:void(0) onClick="openwin('editADServiceMaster?adServiceId='+encodeURIComponent('<%=encADServiceId%>'))">${theCount.index+1}</a></td>
      	 <td>${obj.userId}</td>
        <td>${obj.ldapUrl}</td>
        <td>${obj.status}</td>
        </tr>
...
function openwin(url){
    var newwindow=open(url,"win","height=385,width=660,left=100,top=100,status=1,scrollbars=yes");
}
</script>
